...
capa_0 = tf.Variable(tf.random_uniform([VENTANA, 10], -1, 1), name='capa0')
capa_1 = tf.Variable(tf.random_uniform([10, 5], -1, 1), name='capa1')
capa_2 = tf.Variable(tf.random_uniform([5, 1], -1, 1), name='capa2')

bias_0 = tf.Variable(tf.zeros([10]), name='bias0')
bias_1 = tf.Variable(tf.zeros([5]), name='bias1')
...
for i in range(150000):
    ...
    if i % 1000 == 0:
        ...
        print('Costo: {}'.format(sesion.run(costo, feed_dict={
            x_: X,
            y_: Y
        })))
...
json.dump({
    'ws': ws,
    'bs': bs
}, open('datos/BannedCardsPesos.json', 'w'))